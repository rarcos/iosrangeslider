...
- (void)_RS_commonInit;
{
	...
	_trackBackground = [[UIImageView alloc] initWithImage:[[UIImage imageNamed:@"bar-background.png"] stretchableImageWithLeftCapWidth:0.0 topCapHeight:0.0]];
	_trackBackground.frame = CGRectMake(_padding, _trackBackground.frame.size.height / 2.0 + 8.0, bounds.size.width - (_padding * 2.0), _trackBackground.frame.size.height);
	_trackBackground.autoresizingMask = UIViewAutoresizingFlexibleWidth;

	[self addSubview:_trackBackground];

	_track = [[UIImageView alloc] initWithImage:[[UIImage imageNamed:@"bar-highlight.png"] stretchableImageWithLeftCapWidth:0.0 topCapHeight:0.0]];
	_track.frame = CGRectMake(_padding, _track.frame.size.height / 2.0 + 8.0, bounds.size.width - (_padding * 2.0), _track.frame.size.height);
...
- (void)layoutSubviews
{
	...
	_minThumb.center = CGPointMake([self xForValue:selectedMinimumValue], self.frame.size.height / 2.0);
	_maxThumb.center = CGPointMake([self xForValue:selectedMaximumValue], self.frame.size.height / 2.0);
...
- (void)updateTrackHighlight
{
	_track.frame = CGRectMake(_minThumb.center.x, _track.center.y - (_track.frame.size.height / 2.0), _maxThumb.center.x - _minThumb.center.x, _track.frame.size.height);
}
